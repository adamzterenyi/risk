...
class Board(object):
    ...
    def is_valid_attack_path(self, path):
        ...
        def go(xs):
            if len(path) <= 1:
                return False
            neighbors = list(self.neighbors(path[0]))
            neighbors = [self.owner(neighbor.territory_id) for neighbor in neighbors]
    ...
    def shortest_path(self, source, target):
        ...
        while queue:
            ...
            for territory in risk.definitions.territory_neighbors[current_territory]:
                if territory not in visited_territories:
    ...
    def can_fortify(self, source, target):
        ...
        while queue:
            ...
            if current_territory == target:
                return dictionary[current_territory]
            for territory in risk.definitions.territory_neighbors[current_territory]:
                if territory not in visited_territories and self.owner(territory) == pid:
                    visited_territories.add(territory)
                    copy_path = copy.copy(dictionary[current_territory])
                    copy_path.append(territory)
                    dictionary[territory] = copy_path
    ...
    def cheapest_attack_path(self, source, target):
        ...
        while not priority_queue.empty():
            current_priority, current_territory = priority_queue.get()
            if current_territory == target:
                return dictionary[current_territory]
            for territory in risk.definitions.territory_neighbors[current_territory]:
                if territory not in visited_territories and self.owner(territory) != pid:
                    copy_territory = dictionary[current_territory].copy()
                    copy_territory.append(territory)
                    priority = current_priority + self.armies(territory)
                    p = priority
                    t = territory
                    if territory not in [x[1] for x in priority_queue.queue]:
                        dictionary[territory] = copy_territory
                        priority_queue.put((priority, territory))
                    elif p < min([x[0] for x in priority_queue.queue if x[1] == t]):
                        dictionary[territory] = copy_territory
                        for i, (p, t) in enumerate(priority_queue.queue):
                            if t == territory:
                                priority_queue.queue[i] = (priority, territory)
                                break
